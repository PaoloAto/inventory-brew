...
router.post('/:id/receive', async (req, res) => {
  try {
    ...
    for (const line of body.items || []) { const id = typeof line?.purchaseOrderItemId === 'string' ? line.purchaseOrderItemId : ''; const quantity = line?.quantity; const unitCost = line?.unitCost; if (!mongoose.isValidObjectId(id)) details.push('purchaseOrderItemId must be valid'); else if (seen.has(id)) details.push('Duplicate purchase order item lines are not allowed'); else seen.add(id); if (typeof quantity !== 'number' || !Number.isFinite(quantity) || quantity <= 0) details.push('quantity must be a positive number'); if (typeof unitCost !== 'number' || !Number.isFinite(unitCost) || unitCost < 0) details.push('unitCost must be a non-negative number'); items.push({ purchaseOrderItemId: id, quantity, unitCost }) }
